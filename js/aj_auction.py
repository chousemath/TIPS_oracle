...
headers = ['title', 'model_year', 'mileage', 'transmission', 'fuel', 'color', 'submission_number', 'plate',
           'parking_location', 'lane', 'registered', 'rating', 'category', 'starting_price', 'status', 'result', 'final_price']
        ...
        try:
            with open(fpath, 'r') as f:
                ...
                for car in cars:
                    ...
                    if '경매결과' not in t:
                        continue
                    data = [x.strip() for x in t.replace('  ', '').replace(
                        '\t', '').split('\n') if x.strip()]
                    data = [x for x in data if x not in removed]
                    result = data[:15] + data[16:]
                    if len(result) < 17:
                        result.append('0')
                    result[0] = ' '.join(result[0].split(
                        ' ')[1:]).strip()  # clean up the title
                    result[1] = int(result[1])
                    result[2] = int(result[2].replace(
                        ',', '').replace('km', '').strip())
                    ...
                    result[13] = int(result[13].replace(',', '').replace(
                        '"', '').replace('시작가 :', '').strip())
                    result[14] = result[14].replace('진행상태 :', '').strip()
                    result[16] = int(result[16].replace(',', '').replace(
                        '희망가 :', '').replace('(', '').replace(')', '').strip())
