...
df_encar = df_encar[(df_encar.plate != '') & (df_encar.price > 0) & (df_encar.timestamp > 0)]
df_encar['name'] = df_encar['title'].astype(str) + df_encar['model_year'].astype(str)

names = [(norm(x), len(df_encar[df_encar.name == x])) for x in df_encar.name.unique()]
...
plt.savefig(path.join('experiments', 'process_encar_02', 'timeseries.png'), bbox_inches = "tight")
