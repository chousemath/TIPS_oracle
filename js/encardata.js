        ...
        const scrapeEncar = async (pageLimit, pageLink) => {
            ...
            for (let pageLink of pageLinks) {
                try {
                    ...
                    for (let detailLink of detailPageLinks) {
                        try {
                            ...
                            const getDetailImages = await page.evaluate(() => {
                                ...
                                if (optionsExtra.length > 0)
                                    details.optionsExtra = optionsExtra[0].innerText.trim();
                                ...
                                if (insurance.length > 0)
                                    details.insurance = insurance[0].innerText.trim();
                                details.insuranceReportDate = '';
                                const insuranceReportDate = Array.from(document.getElementsByClassName('ins_notice'));
                                if (insuranceReportDate.length > 0)
                                    details.insuranceReportDate = insuranceReportDate[0].innerText.split('\n')[0].replace(/(\r\n|\n|\r)/gm, '').replace(/ /g, '');
                                details.vehicleDetails = '';
                                const vehicleDetails = document.getElementById('areaCardetail');
                                if (vehicleDetails)
                                    details.vehicleDetails = vehicleDetails.innerText;
                            ...
                            */
                            ...
                            if (data.colorCode === 0) {
                                ...
                                if (!doc) {
                                    ...
                                    collectionSortingRequired.insertOne(payload, (err, res) => {
                                        if (err)
                                            console.log(`MongoDB Error (sorting required): ${err}`);
                                        else
                                            console.log(`Problematic color (${data.color}) saved`);
                                    });
                                }
                            }
                            data.images = lodash_1.default.uniq(data.images);
                            if (data.inspectionPerformed) {
                                ...
                                const getInspectionDetails = await page.evaluate(() => {
                                    ...
                                    inspect.carInfo = Array.from(document.getElementsByClassName('inspec_carinfo'))[0].innerText;
                                    const comprehensiveStatusLabels = ["주행거리 계기상태", "주행거리 상태", "차대번호 표기", "배출가스", "튜닝", "특별이력", "용도변경", "색상", "주요옵션"];
                                    const comprehensiveStatus = Array.from(document.getElementsByClassName('tbl_total')[0]
                                    ...
                                    const repairStatus = Array.from(document.getElementsByClassName('tbl_repair')[0]
                                        .getElementsByTagName('tr'))
                                        .map(tr => Array.from(tr.getElementsByTagName('span'))
                                        .filter(span => {
                                        const className = span.className;
                                    ...
                                    if (comments.length > 0)
                                        inspect.comments = comments[0].innerText;
                    ...
                    console.error('Error navigating to list page', error);
                }
            }
            await browser.close();
        };
